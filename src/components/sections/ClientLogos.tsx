...
export function ClientLogos() {
  ...
  return (
    <section className="py-16 sm:py-20 md:py-24 bg-background relative z-10 overflow-hidden">
      {/* Trust stat bar */}
      <motion.div
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 max-w-3xl mx-auto">
          {TRUST_ITEMS.map((item, i) => (
            ...
            >
              <div className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground">{item.value}</div>
      ...
      <div className="relative">
        ...
        >
          {logos.map((client, i) => (
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2 sm:py-3 rounded-full border border-border group-hover:border-primary/30 transition-colors bg-card/30 group-hover:bg-card/60">
                {/* Initials avatar */}
                <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-secondary flex items-center justify-center text-[10px] sm:text-xs font-bold text-muted-foreground group-hover:text-foreground group-hover:bg-primary/10 transition-colors flex-shrink-0">
